...
const CATEGORIES = [
  {
    ...
    kpis: [
      { id: 'cs_responsiveness', label: 'Responsiveness', question: 'How promptly does this employee respond to customer inquiries?' },
      { id: 'cs_satisfaction', label: 'Customer Satisfaction', question: 'What is the overall customer satisfaction level?' }
    ]
  },
  {
    name: 'Work Quality',
    kpis: [
      { id: 'wq_accuracy', label: 'Accuracy', question: 'How accurate is their work?' },
      { id: 'wq_completeness', label: 'Completeness', question: 'Do they complete tasks comprehensively?' }
    ]
  },
  {
    name: 'Teamwork',
    kpis: [
      { id: 'tw_collaboration', label: 'Collaboration', question: 'How well do they collaborate with colleagues?' },
      { id: 'tw_communication', label: 'Communication', question: 'How clear is their communication?' }
    ]
  },
  {
    name: 'Initiative',
    kpis: [
      { id: 'in_proactive', label: 'Proactive Approach', question: 'How proactive are they in identifying improvements?' }
    ]
  },
  {
    name: 'Professional Development',
    kpis: [
      { id: 'pd_learning', label: 'Continuous Learning', question: 'How committed are they to continuous learning?' }
    ]
  }
];
...
export default function LeaderReviewForm() {
  ...
  useEffect(() => {
    ...
    if (parsedUser.role !== 'leader' && parsedUser.role !== 'admin') {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.length > 0) {
          ...
          loadFormData(data[0].id, new Date().toISOString().slice(0, 7));
        }
      }
    } catch (error) {
  ...
  const loadFormData = async (employeeId: string, month: string) => {
    try {
      const { data } = await supabase
  ...
  const handleEmployeeChange = async (employeeId: string) => {
    ...
    await loadFormData(employeeId, currentMonth);
  };
  ...
  const autoSaveForm = async () => {
    if (!user || !currentMonth || !selectedEmployee) return;

    try {
      const { data: existing } = await supabase
        .from('leader_review_submissions')
        .select('id')
        .eq('reviewed_employee_id', selectedEmployee)
      ...
      const payload = {
        reviewed_employee_id: selectedEmployee,
        reviewed_employee_name: selectedEmp?.name || '',
        reviewer_id: user.id,
    ...
    <div style={{background: 'linear-gradient(135deg, #f0f4f8 0%, #d9e2ec 100%)', minHeight: '100vh'}}>
      ...
      <div style={{background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 100%)', borderBottom: '1px solid rgba(30, 58, 95, 0.08)', boxShadow: '0 2px 12px rgba(0, 0, 0, 0.04)', position: 'sticky', top: 0, zIndex: 100}}>
        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', maxWidth: '1600px', margin: '0 auto', padding: '0 32px', height: '70px'}}>
          <Link href="/dashboard" style={{display: 'flex', alignItems: 'center', gap: '12px', textDecoration: 'none'}}>
            ...
            <div style={{fontSize: '18px', fontWeight: '800', color: '#1e3a5f'}}>Leader Review</div>
          </Link>
      ...
      <div style={{padding: '48px 32px', maxWidth: '1200px', margin: '0 auto'}}>
        <div style={{marginBottom: '40px'}}>
          <h1 style={{fontSize: '36px', fontWeight: '800', color: '#0f172a', marginBottom: '8px'}}>Leader Performance Review</h1>
        ...
        {/* Employee Selector */}
        <div style={{marginBottom: '40px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', borderRadius: '14px', padding: '24px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.04)', border: '1px solid rgba(30, 58, 95, 0.06)'}}>
          <label style={{display: 'block', fontSize: '14px', fontWeight: '700', color: '#1e3a5f', marginBottom: '12px'}}>Select Employee to Review</label>
          <select
        ...
        <div style={{display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #e2e8f0'}}>
          ...
          <button onClick={submitForm} disabled={submitting} style={{padding: '12px 32px', border: 'none', borderRadius: '10px', background: 'linear-gradient(135deg, #7eb8d4, #6ba3c5)', color: 'white', fontWeight: '600', fontSize: '14px', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.6 : 1, transition: 'all 0.3s', boxShadow: '0 4px 12px rgba(126, 184, 212, 0.3)'}}>
